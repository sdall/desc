...
function log_expectation_ts!(p::MaxEnt, x, xbuffer, buffer, ctx; tid=Threads.threadid())
    log_expectation!(p, copy!(xbuffer[tid], x), buffer[tid], ctx[tid])
end
function expectation_ts!(p::MaxEnt, x, xbuffer, buffer, ctx; tid=Threads.threadid())
    expectation!(p, copy!(xbuffer[tid], x), buffer[tid], ctx[tid])
end
isforbidden_ts!(p::MaxEnt, x, max_size, max_width, A; tid=Threads.threadid()) = !isallowed!(p, copy!(A[tid], x), max_size, max_width)
function isforbidden_ts!(ps::Vector{MaxEnt{S, T}}, x, max_size, max_width, A; tid=Threads.threadid()) where {S, T}
    !all(ps) do p
        isallowed!(p, copy!(A[tid], x), max_size, max_width)
    end
end